#!/bin/bash
# glim_rosbag_docker_indoor_cpu.sh
...
# 必要なディレクトリが存在することを確認
if [ ! -d "$HOME/glim_docker/config_L2_indoor_cpu" ]; then
    echo "エラー: $HOME/glim_docker/config_L2_indoor_cpu ディレクトリが見つかりません"
    exit 1
fi
...
# Docker コンテナを起動
echo "GLIM Indoor CPU rosbag処理を開始します..."
...
docker run \
  ...
  -v "$HOME/glim_docker/config_L2_indoor_cpu":/root/ros2_ws/install/glim/share/glim/config:ro \
