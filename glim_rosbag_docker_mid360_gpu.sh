...
# 必要なディレクトリが存在することを確認
if [ ! -d "$HOME/glim_docker/config_mid360" ]; then
    echo "エラー: $HOME/glim_docker/config_mid360 ディレクトリが見つかりません"
    exit 1
fi
...
docker run \
  ...
  -v "$HOME/glim_docker/config_mid360":/root/ros2_ws/install/glim/share/glim/config:ro \
