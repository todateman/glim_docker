...
# 必要なディレクトリが存在することを確認
if [ ! -d "$HOME/glim_docker/config_L2_indoor_gpu" ]; then
    echo "エラー: $HOME/glim_docker/config_L2_indoor_gpu ディレクトリが見つかりません"
    exit 1
fi
...
docker run \
  ...
  -v "$HOME/glim_docker/config_L2_indoor_gpu":/root/ros2_ws/install/glim/share/glim/config:ro \
